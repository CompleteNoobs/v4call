...
- **Rate post precision floor: any rate below 0.001 is treated as free.** Server-side checks like `if (paid >= 0.001 && textMemo && msgId)` and `if (textRate >= 0.001)` use `0.001` as the threshold for "this is paid." For high-precision tokens like SWAP.BTC (8 decimal places), users testing with very small amounts (e.g. 0.00000001 SWAP.BTC) will see the rate apply as **free**. Bump the test amount to ≥ 0.001 of the token. Production amounts for SWAP.BTC are normally much higher than this anyway (0.001 SWAP.BTC ≈ $50–100 USD depending on BTC price). Discovered by the user during SWAP.BTC ring-fee testing on 2026-05-04. **Confusing UX symptom**: the picker shows a paid option with the small rate (because `get-all-rates` returns any non-zero option), but the actual call/DM goes through as free (because the server-side `>= 0.001` check fails). Same surface, two different code paths reach different conclusions. **Future fix is now elevated to its own planned task** — see "v0.16.8 candidate — Token-aware precision floor" in Planned Features.
...
### v0.16.8 candidate — Token-aware precision floor (small focused fix)
The hardcoded `>= 0.001` floor in server-side rate checks (`if (paid >= 0.001 && textMemo && msgId)`, `if (textRate >= 0.001)`, etc.) was chosen for HBD precision (3 decimals). It's wrong for high-precision tokens like SWAP.BTC (8 decimals = 1 satoshi minimum). Symptom: rate post with a small SWAP.BTC ring fee (e.g. 0.000001) is shown by the picker as a paid option, but server treats the actual call/DM as free because the threshold check fails. Confusing UX — picker and validator disagree.

**Fix shape:**
- Lookup the token's precision from Hive-Engine token metadata (cached). For SWAP.BTC: 8 decimals → floor = 0.00000001 (1 satoshi). For HBD/HIVE: 3 decimals → floor = 0.001 (current value).
- Replace hardcoded `0.001` thresholds with `getTokenPrecisionFloor(currency)` calls.
- Audit all `>= 0.001` literals in server.js — there are several (lobby-dm handler, federation dm handler, payment-verified handler).
- Update `computePaymentOptions` to also use the per-currency floor when filtering out "free" rates.

**Estimated scope:** 1 short session. Important to land BEFORE v0.17 paid-expert-invites adds another paid flow that would inherit the same bug.

**Discovered by user during SWAP.BTC ring-fee testing 2026-05-04.** Caveat: in production, SWAP.BTC ring fees would normally be 0.001 SWAP.BTC or more (≈ $50–100 USD), so this primarily affects test setups + small-amount tipping use cases. Still worth fixing — the picker/validator disagreement is the surprising part.
