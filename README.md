...
**Current version**
- Software **v0.16** — Cross-server rooms, end-to-end. **Part A** (already shipped): admin types `@user@peer.com` into a room's allowlist, federated invite + accept/decline handshake. **Part B** (this build): accepting actually joins the room. Joiner's browser opens a temp Socket.io to the host server; multi-party WebRTC works; federated members appear with a `@hive-book.com` badge in the user-list and on video tiles; token-gating works across federation; banlist auto-kick honours canonical form; accepting a federated invite auto-leaves any current local room first; federation drop while mid-room → immediate eviction (no grace period for rooms). Builds on v0.15 (spotlight + local pin + admin spotlight broadcast + admin role transfer + Share Screen + WebRTC SDP m-line fix). Production-deployed on [call.completenoobs.com](https://call.completenoobs.com) ↔ [hive-book.com](https://hive-book.com).
- Federation protocol **v0.4** — first wire-format bump since v0.11. New `room-invite` / `room-response` envelopes (generic `payload: {}` for forward-compat with v0.17 paid expert invites) plus an explicit `protocol_version: '0.4'` field in the hello, so older v0.3 peers continue to federate fully for everything they could do at v0.3. Cross-server room *join* itself reuses the existing direct browser↔host-server Socket.io transport (the same one 1:1 federated calls already use) — no extra federation envelope for the join.
...
| ~~**v0.16 / fed v0.4**~~ ✅ shipped | Cross-server rooms, end-to-end. **Part A**: `room-invite` / `room-response` envelopes + `protocol_version: '0.4'` gate + federated allowlist input (`@user@peer.com`) + accept/decline handshake. **Part B**: actual cross-server room join via temp Socket.io (mirrors 1:1 federated calls), federated badge in user-list + tile labels, federated token-gating (chain-side, peer-agnostic), banlist auto-kick by canonical form, federation peer drop → immediate eviction of federated members from all rooms |
