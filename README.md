...
**Current version**
- Software **v0.16.5** — Lobby DM bypass fix. Removed the `lobby-encrypted` socket event entirely (was a "send encrypted lobby message to selected users" path that bypassed paid-DM rate enforcement, blocked-list, platform fee minimum, and recipient currency / token-gate / time-window rules — including across federation, where it was forwarded as a free `dm` envelope with `textPaid: 0`). Lobby chat is now broadcast-only (signed, public). User-list toggle is single-purpose: pre-select for "Create Room" invite. Server keeps a deprecation stub that emits a clear rejection if a stale client tries the old event. **Important rollout note:** the bypass happens on the *sender's* server, so all federated peers should upgrade to v0.16.5 in close sequence to fully close it. The architectural fix that closes the broader caller-side-trust class lands in v0.16.6. Builds on v0.16 — Cross-server rooms, end-to-end (type `@user@peer.com` into a room's allowlist; federated invite + accept/decline; multi-party WebRTC; federated badge; token-gating + banlist + federation-peer-drop eviction across the federation; XSS hygiene pass). Production-deployed on [call.completenoobs.com](https://call.completenoobs.com) ↔ [hive-book.com](https://hive-book.com) ↔ [v4call.com](https://v4call.com).
- Federation protocol **v0.4** — unchanged in v0.16.5 (server-local fix, no wire-format change). v0.4 introduced `room-invite` / `room-response` envelopes (generic `payload: {}` for forward-compat with v0.17 paid expert invites) plus an explicit `protocol_version: '0.4'` field in the hello, so older v0.3 peers continue to federate fully for everything they could do at v0.3. Cross-server room *join* itself reuses the existing direct browser↔host-server Socket.io transport (same as 1:1 federated calls) — no extra federation envelope for the join.
...
| **v0.16.6** | **Recipient-side rate enforcement** — apply "recipients enforce their own rules" pattern. Re-fetch recipient's rates + re-validate block-list, rate, platform fee minimum, currency on the recipient's server when handling federated paid DMs and paid calls. Closes the caller-side-trust class of bypasses. No protocol bump. Lands before v0.17 so paid-expert-invites build on a secure foundation. |
