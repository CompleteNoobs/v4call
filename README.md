...
- **Paid room invites (v0.16.10–v0.16.13):** Set an `INVITE:` rate on your rate post (per token / per named-list window / per default window) — anyone not matched by a free list pays that fee to invite you to a private room. Inviter sees a Keychain payment modal; on success the server records, charges, and forwards the invite with a "💜 Paid invite — X paid Y CUR" badge in your popup. You keep the net (minus the server's platform fee); declining or not responding within 15 minutes auto-refunds the inviter. Same-server and cross-federation both supported (federation closed in v0.16.11; room-create federated bypass closed in v0.16.13 — paid federated invitees during Create Room are deferred to the in-room "Paid invites pending" notice with a one-click "Invite →" pre-fill of the allowlist panel).
...
- **Cross-server paid invites (v0.16.11+, no protocol bump):** inviter-holds-funds model — payment lands in the SOURCE server's escrow (inviter's home server), source forwards `room-invite` with `payload: { payment: { currency, paid, memo, source_escrow } }`, recipient server re-validates rates + on-chain verifies the payment landed in the claimed escrow before delivering the popup. Source server settles on response: accept → cross-Hive disburse to invitee + platform fee to source operator; decline / offline auto-decline / recipient-side `paid_rejected` / 15-min timeout → refund inviter. Inverts the paid-DM federation pattern deliberately — protects against rug-pull where a malicious recipient operator could rotate escrow keys mid-flow. To fully close the bypass mesh-wide, all peers in your federation need v0.16.11+; mixed-version peers still function but pre-v0.16.11 inviters generate free `payload: {}` envelopes that the v0.16.13 recipient validates as free.
