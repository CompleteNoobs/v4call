...
- **Wire up the 🖥️ Share Screen button** (placeholder added in v0.13). Implementation: `navigator.mediaDevices.getDisplayMedia({ video: true })`, swap the video sender's track via `RTCRtpSender.replaceTrack(screenTrack)` (no renegotiation needed if we reuse the cam track's sender), listen for `screenTrack.onended` so the browser-initiated "Stop sharing" prompt resets our button state. Mutually exclusive with cam (only one video source at a time). Promotes the screen-sharer into the spotlight automatically. iOS Safari/Brave doesn't support `getDisplayMedia` — gate the button on capability detection there.
