...
- **Software:** v0.16 (Part A) — Cross-server room invites. Admin types `@user@peer.com` into the room's allowlist input (or types it after the leading `@`, e.g. `@cnoobz@hive-book.com`); server detects the federated form, validates the target server is an approved + connected v0.4+ peer, sends a `room-invite` over federation. Receiving server delivers the popup with a source-server badge; user accepts/declines; response flows back as `room-response`. Cross-server *join* itself lands in v0.16 Part B — for v0.16 Part A, accept just confirms receipt with a "Cross-server join arrives in Part B" system message. Federated allowlist entries are stored canonically as `user@server.com` and rendered with a small server badge in the allowlist panel. Pending invites prune after 15 minutes. Builds on v0.15 (spotlight room layout + local pin + admin spotlight broadcast + admin role transfer + Share Screen + WebRTC SDP m-line fix). Production-deployed on call.completenoobs.com ↔ hive-book.com.
- **Federation protocol:** v0.4 (Part A) — first wire-format bump since v0.11. New `room-invite` / `room-response` envelopes (generic `payload: {}` for forward-compat with v0.17 paid expert invites) and an explicit `protocol_version: '0.4'` field in the hello. v0.3 peers continue to federate fully for everything they could do at v0.3; v0.4-only features (cross-server room invites) are gated by the `protocol_version` field. Verify.json domain proof, Hive-tag discovery, manual peer approval, paid cross-server calls + DMs all unchanged.
...
- ⏳ Cross-server room join: callee's browser opens temporary cross-server Socket.io to caller's server (same pattern as 1:1 federated calls already work) — **Part B**
- ⏳ Token-gating works across federation (Hive-Engine balance check is chain-side, peer-agnostic) — **Part B** (drops in for free once Part B's join machinery lands)
- ⏳ Federation message added: `room-ended` (cleanup when host destroys the room or bans a federated member) — **Part B**
- **Forward-compat reminder for Part B:** the cross-server room-join flow should track an optional per-user "billing context" alongside the join (null for free invitees, an `activePayments` reference for paid invitees in v0.17).
