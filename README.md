...
**Current version**
- Software **v0.16** — Cross-server rooms, end-to-end. Type `@user@peer.com` into a room's allowlist (or tick federated users in the lobby user-picker before clicking Create Room) — federated invite + accept/decline handshake fires over the federation socket; on accept, the joiner's browser opens a temp Socket.io to the host server and joins as a real participant. Multi-party WebRTC; federated members appear with a `@hive-book.com` badge in the user-list and on video tiles; token-gating works across federation (chain-side balance check); banlist auto-kick honours canonical `user@server` form; accepting auto-leaves any current local room first; federation peer drop while mid-room → immediate eviction (no grace period for rooms). Plus an XSS hygiene pass — `data-action` event delegation replaces dynamic inline onclicks across rooms, allowlist, banlist, and lobby user-list. Builds on v0.15 (spotlight + local pin + admin spotlight broadcast + admin role transfer + Share Screen + WebRTC SDP m-line fix). Production-deployed on [call.completenoobs.com](https://call.completenoobs.com) ↔ [hive-book.com](https://hive-book.com).
